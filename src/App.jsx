...
import { Chart, LineElement, BarElement, CategoryScale, LinearScale, PointElement, Filler, Tooltip } from "chart.js";
...
function WaterfallCanvas({rows,selCh,onChClick}) {
  ...
  useRef(() => {}); // dummy
  // draw every render — simple approach
  const draw = useCallback((canvas) => {
    ...
    const rH=Math.max(1.5,H/WF_MAX);
    const cW=W/18;
    rows.forEach((row,ri)=>{
    ...
    ctx.strokeRect(selCh*cW,0,cW,H);
    ctx.globalAlpha=1;
    // channel dividers
    ...
    CH.filter((_,i)=>i%3===0).forEach((ch,j)=>{
      ctx.fillStyle="#3d5c7a"; ctx.font="9px monospace";
      ctx.fillText(ch.nm,j*3*cW+4,H-4);
    });
  }, [rows,selCh]);
  ...
  return (
    <canvas
      ...
      onClick={(e)=>{
        ...
        const idx=Math.floor(x/rect.width*18);
        if(idx>=0&&idx<18)onChClick(idx);
      }}
    />
  );
}
...
function ScoreArc({score,size=88}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
      ...
      <span style={{fontSize:10,fontWeight:700,letterSpacing:.8,color:col,textTransform:"uppercase"}}>
        {score==null?"awaiting":score>=70?"good":score>=45?"moderate":"poor"}
      </span>
    </div>
  );
}

// ─── gauge bar with range markers ────────────────────────────────────────────
function GaugeBar({label,value,max,color,unit,status,statusColor,lo,hi}) {
  ...
  const loPct=lo!=null?lo/max*100:null;
  const hiPct=hi!=null?hi/max*100:null;
  return (
...
// ─── channel card (18-channel view) ──────────────────────────────────────────
function ChCard({ch,idx,val,hist,selected,onClick}) {
  const pct=Math.round(val/4095*100);
  return (
    <div onClick={onClick} style={{
      background:"#0a1020",borderRadius:10,padding:"10px 10px 8px",cursor:"pointer",
      border:selected?`2px solid ${ch.col}`:"1px solid #172136",
      transition:"border-color .12s, background .12s",
      boxShadow:selected?`0 0 12px ${rgba(ch.col,.2)}`:"none",
    }}>
...
export default function App() {
  ...
  async function connect() {
    ...
    try {
      ...
      log("Waiting for: RAW:v1,v2,...,v18","info");
        ...
        for(const raw of lines){
          ...
          if(vals.length===18&&vals.every(v=>!isNaN(v)&&v>=0&&v<=65535)){
            if(!pauseRef.current){
              ...
              n++;setReads(n);
              if(n%10===0){
                const s=calcSoil(corr);
                const snap={t:new Date().toLocaleTimeString("en-GB"),moisture:s?.moisture,score:s?.score,N:s?.N,P:s?.P,K:s?.K};
                snapRef.current=[...snapRef.current.slice(-29),snap];
                setSnaps([...snapRef.current]);
              }
  ...
  async function disconnect(){
    ...
    try{if(rdrRef.current){await rdrRef.current.cancel();rdrRef.current=null;}if(portRef.current){await portRef.current.close();portRef.current=null;}}catch{}
  }
  ...
  const specOpts={
    ...
    plugins:{legend:{display:false},tooltip:{callbacks:{title:ctx=>CH[ctx[0].dataIndex].nm+" nm — "+CH[ctx[0].dataIndex].role,label:ctx=>"  ADC: "+ctx.parsed.y}}},
  ...
  const trendOpts={
    ...
    scales:{x:{display:false},y:{min:0,max:4095,ticks:{color:"#3d5c7a",font:{size:9},maxTicksLimit:4},grid:{color:"#111e33"},border:{display:false}}},
  };

  // moisture history
  const moistChart={
    labels:snapshots.map(s=>s.t),
    datasets:[
      {label:"Moisture %",data:snapshots.map(s=>s.moisture??null),borderColor:"#38bdf8",backgroundColor:rgba("#38bdf8",.08),borderWidth:2,pointRadius:2,pointBackgroundColor:"#38bdf8",tension:.4,fill:true,spanGaps:true},
      {label:"Score",data:snapshots.map(s=>s.score??null),borderColor:"#22c55e",backgroundColor:rgba("#22c55e",.05),borderWidth:1.5,pointRadius:0,tension:.4,fill:false,spanGaps:true,yAxisID:"y2"},
    ],
  };
  const moistOpts={
    responsive:true,maintainAspectRatio:false,animation:{duration:0},
    ...
    scales:{
      x:{ticks:{color:"#3d5c7a",font:{size:9},maxTicksLimit:8},grid:{color:"#111e33"},border:{display:false}},
      y:{min:0,max:100,ticks:{color:"#38bdf8",font:{size:9},maxTicksLimit:5},grid:{color:"#111e33"},border:{display:false}},
      y2:{position:"right",min:0,max:100,ticks:{color:"#22c55e",font:{size:9},maxTicksLimit:5},grid:{display:false},border:{display:false}},
    },
  };
  ...
  const V = {
    ...
    logo:{fontSize:15,fontWeight:800,color:"#d4e2f4",letterSpacing:.5},
    logoG:{color:"#22c55e"},
    ...
    ss:(extra)=>({padding:"10px 12px",borderBottom:"1px solid #0f1a2d",...extra}),
    ...
    ct:{fontSize:12,fontWeight:700,color:"#d4e2f4"},
    cs:{fontSize:10,color:"#2d4060"},
    // band
    ...
    // tile strip
    tileStrip:{display:"flex",overflowX:"auto",flexShrink:0},
    tile:(sel,col)=>({
      minWidth:54,flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      gap:2,borderRight:"1px solid #14213a",padding:"4px 2px",cursor:"pointer",transition:"background .1s",
      background:sel?"#142236":"transparent",borderTop:sel?`2px solid ${col}`:"2px solid transparent",
    }),
  };
  ...
  const METRICS=[
    {label:"Moisture",val:soil?.moisture,unit:"% VWC",col:"#38bdf8",max:100,
     ...
     sc:!soil?"#2d4060":soil.moisture<25?"#ef4444":soil.moisture<65?"#22c55e":"#f59e0b"},
    {label:"Org. Matter",val:soil?.om,unit:"% SOM",col:"#4ade80",max:15,
     ...
     sc:!soil?"#2d4060":parseFloat(soil?.om)<2?"#ef4444":parseFloat(soil?.om)<5?"#f59e0b":"#22c55e"},
    {label:"Conductivity",val:soil?.ec,unit:"mS/cm",col:"#22d3ee",max:6,
     ...
     sc:!soil?"#2d4060":parseFloat(soil?.ec)<0.8?"#f59e0b":parseFloat(soil?.ec)<3?"#22c55e":"#ef4444"},
    {label:"NIR Index",val:soil?.ndmi,unit:"NDMI",col:"#a78bfa",max:1,
     ...
     sc:!soil?"#2d4060":parseFloat(soil?.ndmi)>0.1?"#22c55e":parseFloat(soil?.ndmi)<-0.2?"#ef4444":"#f59e0b"},
  ];
  const NPK=[{el:"N",val:soil?.N,max:350,col:"#4ade80"},{el:"P",val:soil?.P,max:160,col:"#fb923c"},{el:"K",val:soil?.K,max:280,col:"#c084fc"}];

  return (
    <div style={V.page}>
      <style>{`@keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:#080c18}::-webkit-scrollbar-thumb{background:#1e3a5f;border-radius:2px}button:hover{filter:brightness(1.15)}`}</style>
      ...
      <div style={V.top}>
        ...
        <div style={V.tabs}>
          {[["dashboard","Dashboard"],["channels","18 Channels"],["analysis","Analysis"],["log","Log"]].map(([k,l])=>(
            <button key={k} style={V.tab(tab===k)} onClick={()=>setTab(k)}>{l}</button>
          ))}
        </div>
      ...
      <div style={V.body}>
        ...
        <div style={V.side}>
          ...
          <div style={V.pb(soil?.ploughReady)}>
            ...
            <div>
              <div style={{fontSize:11,fontWeight:800,color:!soil?"#3d5c7a":soil.ploughReady?"#22c55e":"#ef4444",marginBottom:1}}>
                {!soil?"Awaiting data":soil.ploughReady?"Ready to Plough":"Not Ready to Plough"}
              </div>
              <div style={{fontSize:10,color:"#4a6080"}}>
                {!soil?"Connect sensor":soil.ploughReady?"Moisture & EC in tillage range":"Check moisture / EC / OM"}
              </div>
            </div>
          </div>

          {/* soil type + mini metrics */}
          {soil&&(
            <div style={V.ss({})}>
              ...
              <div style={V.sg}>
                {[
                  {l:"Compaction",v:soil.compaction+"/100",col:soil.compaction>60?"#ef4444":soil.compaction>35?"#f59e0b":"#22c55e"},
                  {l:"Salinity",v:soil.salinityRisk,col:soil.salinityCol},
                  {l:"Red-Edge",v:Math.round(soil.nirAvg/40.95)+"%",col:"#f43f5e"},
                  {l:"NIR/VIS",v:(soil.nirAvg/(soil.visAvg||1)).toFixed(2),col:"#a78bfa"},
                ].map(({l,v,col})=>(
                  <div key={l} style={V.sc2}>
                    <div style={V.scl}>{l}</div>
                    <div style={V.scv(col)}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={V.ss({})}>
            ...
            {NPK.map(({el,val,max,col})=>{
              ...
              return(
                <div key={el} style={V.nr}>
                  <span style={V.nel(col)}>{el}</span>
                  <div style={V.ntr}><div style={V.nf(col,pct)}/></div>
                  <span style={V.nppm(col)}>{val!=null?val+" ppm":"—"}</span>
                  <span style={V.nlvl(lc)}>{lvl}</span>
                </div>
              );
            })}
            <div style={{fontSize:8,color:"#1e3a5f",marginTop:2}}>* Spectral proxy — verify with lab analysis before field application</div>
          </div>
          ...
          {soil&&soil.recs.length>0&&(
            <div style={V.ss({})}>
              ...
              {soil.recs.map((r,i)=>(
                <div key={i} style={V.ri(r.type)}>
                  <span style={{fontSize:13,flexShrink:0}}>{r.icon}</span>
                  <span style={{fontSize:10,color:"#94a3b8",lineHeight:1.5}}>{r.text}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{flex:1,overflowY:"auto",padding:"6px 10px 10px"}}>
            ...
            {logs.map((e,i)=>(
              <div key={i} style={{display:"flex",gap:7,fontSize:9,fontFamily:"monospace",padding:"1px 0"}}>
                <span style={{color:"#14213a",flexShrink:0}}>{e.ts}</span>
                <span style={{color:logCol[e.t]}}>{e.msg}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={V.main}>
          ...
          {tab==="dashboard"&&<>
            {/* Waterfall */}
            <div style={{...V.card,display:"flex",flexDirection:"column",gap:0,padding:0,overflow:"hidden",flex:"0 0 auto",height:260}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 14px",borderBottom:"1px solid #0f1a2d"}}>
              ...
              <div style={{flex:1,position:"relative"}}>
                <WaterfallCanvas rows={wfRows} selCh={selCh} onChClick={setSelCh}/>
              </div>
            </div>

            {/* Spectrum bar + trend in a row */}
            <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:12}}>
                ...
                <div style={V.ch}>
                  ...
                  <div style={{display:"flex",gap:12}}>
                    {[["VIS","#fb923c"],["R-Edge","#f43f5e"],["NIR","#a78bfa"]].map(([l,c])=>(
                      <span key={l} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:"#3d5c7a"}}>
                        <span style={{width:10,height:7,borderRadius:2,background:c}}/>{l}
                      </span>
                    ))}
                  </div>
                </div>
                <div style={{position:"relative",height:150}}>
                  <Bar data={specBar} options={specOpts} role="img" aria-label="18 channel ADC bar chart"/>
                </div>
              </div>
              ...
              <div style={V.card}>
                ...
                <div style={{position:"relative",height:110}}>
                  <Line data={trendLine} options={trendOpts} role="img" aria-label="Channel trend"/>
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:8}}>
                  {CH.map((ch,i)=>(
                    <button key={i} onClick={()=>setSelCh(i)} style={{padding:"2px 6px",borderRadius:3,border:"none",cursor:"pointer",fontSize:9,fontWeight:700,background:selCh===i?rgba(ch.col,.25):"transparent",color:selCh===i?ch.col:"#2d4060",outline:selCh===i?`1px solid ${rgba(ch.col,.35)}`:"none"}}>
                      {ch.nm}
                    </button>
                  ))}
                </div>
            ...
            <div style={{...V.card,padding:"8px 4px"}}>
              ...
              <div style={{display:"flex",overflowX:"auto"}}>
                {CH.map((ch,i)=>(
                  <div key={i} onClick={()=>setSelCh(i)} style={V.tile(selCh===i,ch.col)}>
                    <span style={{fontSize:8,color:"#3d5c7a"}}>{ch.nm}nm</span>
                    <span style={{width:20,height:4,borderRadius:1,background:ch.col,flexShrink:0}}/>
                    <span style={{fontSize:11,fontWeight:700,color:ch.col,fontFamily:"monospace"}}>{adc[i]}</span>
                    <span style={{fontSize:7,color:"#2d4060",letterSpacing:.3,textTransform:"uppercase"}}>{ch.r}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {tab==="channels"&&(
            <div style={V.card}>
              <div style={V.ch}>
                <span style={V.ct}>All 18 Channels — 3 rows × 6</span>
                <span style={V.cs}>click any card to view trend on Dashboard</span>
              </div>

              {/* Row 1: 410–535 */}
              <div style={V.bh}>
                <span style={V.bt}>Row 1 — Violet to Green</span>
                <span style={V.bs}>410 · 435 · 460 · 485 · 510 · 535 nm</span>
                <div style={V.bl}/>
              </div>
              <div style={{...V.g6,marginBottom:14}}>
                {[0,1,2,3,4,5].map(i=><ChCard key={i} ch={CH[i]} idx={i} val={adc[i]} hist={hist[i]} selected={selCh===i} onClick={()=>{setSelCh(i);setTab("dashboard");}}/>)}
              </div>

              {/* Row 2: 560–705 */}
              <div style={V.bh}>
                <span style={V.bt}>Row 2 — Yellow to Red-Edge</span>
                <span style={V.bs}>560 · 585 · 610 · 645 · 680 · 705 nm</span>
                <div style={V.bl}/>
              </div>
              <div style={{...V.g6,marginBottom:14}}>
                {[6,7,8,9,10,11].map(i=><ChCard key={i} ch={CH[i]} idx={i} val={adc[i]} hist={hist[i]} selected={selCh===i} onClick={()=>{setSelCh(i);setTab("dashboard");}}/>)}
              </div>

              {/* Row 3: 730–940 */}
              <div style={V.bh}>
                <span style={V.bt}>Row 3 — Near-Infrared</span>
                <span style={V.bs}>730 · 760 · 810 · 860 · 900 · 940 nm</span>
                <div style={V.bl}/>
              </div>
              <div style={V.g6}>
                {[12,13,14,15,16,17].map(i=><ChCard key={i} ch={CH[i]} idx={i} val={adc[i]} hist={hist[i]} selected={selCh===i} onClick={()=>{setSelCh(i);setTab("dashboard");}}/>)}
              </div>
            </div>
          )}
          ...
          {tab==="analysis"&&<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
              <div style={{...V.card,borderTop:"2px solid #22d3ee"}}>
                <div style={{fontSize:10,color:"#3d5c7a",fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:.8}}>Soil Classification</div>
                <div style={{fontSize:22,fontWeight:800,color:"#22d3ee",marginBottom:4}}>{soil?.soilType??"—"}</div>
                <div style={{fontSize:11,color:"#3d5c7a",marginBottom:10}}>Confidence: <b style={{color:"#22d3ee",fontFamily:"monospace"}}>{soil?.soilConf??0}%</b></div>
                <div style={{fontSize:10,color:"#2d4060",lineHeight:1.7}}>Derived from NIR/VIS ratio, organic matter index, and moisture profile across all 18 spectral channels during active ploughing.</div>
              </div>
              <div style={{...V.card,borderTop:"2px solid #ef4444"}}>
                <div style={{fontSize:10,color:"#3d5c7a",fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:.8}}>Compaction Index</div>
                <div style={{fontSize:32,fontWeight:800,fontFamily:"monospace",lineHeight:1,color:soil&&soil.compaction>60?"#ef4444":soil&&soil.compaction>35?"#f59e0b":"#22c55e",marginBottom:8}}>
                  {soil?.compaction??<span style={{color:"#1e3a5f"}}>—</span>}<span style={{fontSize:13,fontWeight:400,color:"#2d4060"}}>/100</span>
                </div>
                <div style={{height:8,background:"#14213a",borderRadius:4,overflow:"hidden",marginBottom:8}}>
                ...
                <div style={{fontSize:11,color:"#4a6080"}}>{!soil?"—":soil.compaction>60?"Deep tillage recommended":soil.compaction>35?"Moderate — standard tillage":"Soil structure good"}</div>
              </div>
              <div style={{...V.card,borderTop:`2px solid ${soil?.salinityCol??"#22c55e"}`}}>
                <div style={{fontSize:10,color:"#3d5c7a",fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:.8}}>Salinity Risk</div>
                <div style={{fontSize:28,fontWeight:800,fontFamily:"monospace",color:soil?.salinityCol??"#1e3a5f",lineHeight:1,marginBottom:6}}>{soil?.salinityRisk??"—"}</div>
                <div style={{fontSize:13,color:"#3d5c7a",marginBottom:6}}>EC: <b style={{color:soil?.salinityCol??"#1e3a5f",fontFamily:"monospace"}}>{soil?.ec??0} mS/cm</b></div>
                <div style={{fontSize:10,color:"#2d4060",lineHeight:1.7}}>
                  {!soil?"—":parseFloat(soil.ec)<0.8?"Low EC — nutrient amendment may help":parseFloat(soil.ec)<3?"EC in optimal range for most crops":"High EC detected — risk of salt stress, leach before sowing"}
                </div>
              </div>
            </div>

            {/* moisture + score trend */}
            <div style={V.card}>
              <div style={V.ch}>
                <span style={V.ct}>Moisture % & Health Score — field trend</span>
                <div style={{display:"flex",gap:14}}>
                  {[["Moisture %","#38bdf8"],["Health Score","#22c55e"]].map(([l,c])=>(
                    <span key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"#3d5c7a"}}><span style={{width:12,height:3,borderRadius:1,background:c}}/>{l}</span>
                  ))}
                </div>
              </div>
              <div style={{position:"relative",height:130}}>
                {snapshots.length>1
                  ? <Line data={moistChart} options={moistOpts} role="img" aria-label="Moisture and health score trend"/>
                  : <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",color:"#1e3a5f",fontSize:11}}>Collecting data — need 2+ snapshots (every 10 reads)</div>
                }
              </div>
            </div>
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr style={{borderBottom:"1px solid #14213a"}}>
                    {["Channel","Region","ADC","% FS","Level","Soil Role"].map(h=>(
                ...
                <tbody>
                  {CH.map((ch,i)=>{
                    ...
                    return(
                      <tr key={i} style={{borderBottom:"1px solid #0c1322",cursor:"pointer",background:selCh===i?"#111e33":"transparent"}} onClick={()=>{setSelCh(i);setTab("dashboard");}}>
                        <td style={{padding:"6px 8px",fontWeight:800,color:ch.col,fontFamily:"monospace",fontSize:12}}>{ch.nm}<span style={{fontSize:9,fontWeight:400,color:"#2d4060"}}> nm</span></td>
                        <td style={{padding:"6px 8px",fontSize:10,color:"#4a6080"}}>{ch.r}</td>
                        <td style={{padding:"6px 8px",fontFamily:"monospace",fontWeight:700,color:"#d4e2f4",fontSize:12}}>{v}</td>
                        <td style={{padding:"6px 8px",width:120}}>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            <div style={{flex:1,height:5,background:"#14213a",borderRadius:2,overflow:"hidden"}}>
                              <div style={{height:"100%",borderRadius:2,background:ch.col,width:pct+"%",transition:"width .3s"}}/>
                            </div>
                            <span style={{fontSize:9,color:ch.col,fontFamily:"monospace",width:24,textAlign:"right"}}>{pct}%</span>
                          </div>
                        </td>
                        <td style={{padding:"6px 8px"}}>
                          <span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:3,background:rgba(ch.col,.12),color:ch.col}}>{pct>75?"HIGH":pct>35?"MED":"LOW"}</span>
                        </td>
          ...
          {tab==="log"&&(
            <div style={V.card}>
              <div style={{...V.ch,marginBottom:8}}>
                <span style={V.ct}>Event Log</span>
                <button onClick={()=>setLogs([])} style={{fontSize:10,color:"#3d5c7a",background:"none",border:"none",cursor:"pointer"}}>Clear</button>
              </div>
              <div style={{fontFamily:"monospace",fontSize:10,lineHeight:1.9,maxHeight:"70vh",overflowY:"auto"}}>
                ...
                {logs.map((e,i)=>(
                  <div key={i} style={{display:"flex",gap:12,padding:"2px 6px",borderRadius:4,background:i%2===0?"transparent":"#080e1c"}}>
                    <span style={{color:"#1e3a5f",flexShrink:0,width:68}}>{e.ts}</span>
                    <span style={{color:logCol[e.t]}}>{e.msg}</span>
                  </div>
                ))}
              </div>
